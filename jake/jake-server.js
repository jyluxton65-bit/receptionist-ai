...
// Always upsert first so the row is guaranteed to be there.
...
app.post('/incoming', (req, res) => {
  ...
  msgQueues[from] = msgQueues[from].then(async () => {
    ...
    if (isClosingMessage(body)) {
      ...
      console.log(`Sign-off sent to ${from}`);
      return;
    }
...
function scheduleBatch(label, csvFile, cronDay, cronHour, cronMinute) {
  ...
  if (isToday && !alreadyDone && inWindow) {
    ...
    if (alreadyDone) console.log(`[Jake] ${label} already fired today - scheduled for next occurrence`);
