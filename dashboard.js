/**
 * Dashboard routes â mounted at /dashboard in server.js
 */
...
<body><div class="card"><div class="logo"><h1>ð³ Grafted Services</h1><p>AI Receptionist Dashboard</p></div>
...
router.get('/api/status', requireAuth, (req, res) => {
  ...
  res.json({ botEnabled: getSetting('bot_enabled') !== 'false', calendarConnected: !!t });
});

router.get('/api/conversations', requireAuth, (req, res) => res.json(getRecentConversations(50)));

router.get('/api/conversations/:phone', requireAuth, (req, res) => {
  res.json(getConversationHistory(decodeURIComponent(req.params.phone), 50));
});
...
router.get('/api/settings', requireAuth, (req, res) => {
  ...
  delete s.google_tokens;
  res.json(s);
});

router.post('/api/settings', requireAuth, express.json(), (req, res) => {
  const allowed = ['bot_enabled', 'day_rate', 'half_day_rate', 'small_job_rate'];
...
router.get('/api/photo-quotes', requireAuth, (req, res) => {
  const limit = parseInt(req.query.limit) || 20;
  res.json(getRecentPhotoQuotes(limit));
});

router.get('/api/photo-quotes/:id/image', requireAuth, (req, res) => {
